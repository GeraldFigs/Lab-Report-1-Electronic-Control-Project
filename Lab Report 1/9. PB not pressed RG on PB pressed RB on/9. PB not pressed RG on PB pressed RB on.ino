/* CODE: RGB colour is yellow when PB is not pressed. When pressed, it turns RGB colour to pink.
...
The logic sets the RGB light yellow when PB is not pressed, as Red and Blue cathodes = Pink.
The colour turns to pink whenever PB is pressed, as Red + Blue = Pink
...
void loop() {                       // Void loop is the section of the program that runs forever.
  ...
  if (buttonState == !0) {          // since internal_pull up inverts the logic, it was inverted by using "!". If statement checks if the button is NOT pressed. then:
   ...
   //TURN THE RGB COLOR YELLOW BY MAKING ONLY THE PINS RED AND GREEN HIGH.
  ...
  //TURN ON ONLY THE RED AND BLUE PIN HIGH.
    analogWrite(red, 255);     // sends a PWM signal with a value of 255 to the pin number stored in the "red" variable, turning on the red LED.
