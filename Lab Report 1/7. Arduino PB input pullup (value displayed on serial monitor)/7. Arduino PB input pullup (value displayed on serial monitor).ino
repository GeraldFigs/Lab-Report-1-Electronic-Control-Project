/*CODE: PB USING ARDUINO'S INTERNAL PULL UP RESISTOR AND BUTTON STATE PRINTED VIA SERIAL MONITOR
...
int PB = 2; //create an integer variable called "PB" and set its value to 2. This is the pin number of the push button.
void setup() {
// put your setup code here, to run once:

Serial.begin(9600);       //initialize the serial communication at a baud rate of 9600.
pinMode (PB, INPUT_PULLUP); // sets pin PB as an input whereas it uses the Arduino's built-in internal pullup resistor.
}

void loop() {
  // put your main code here, to run repeatedly:

int buttonState = digitalRead(PB); // read the PB value and store its data state into a variable
